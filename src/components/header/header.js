...
class Header extends React.Component {
 ...
 componentDidUpdate(prevProps) {
  if (this.props.tag !== undefined && prevProps.tag !== this.props.tag) {
   firebase
    .functions()
    .httpsCallable('tagInfo')({ tag: this.props.tag })
    .then((result) => {
     console.log(result);
     this.setState({
      tag: result,
  ...
  return this.state.loading ? (
   ...
   <>
    <div
     ...
     style={{
      background: `url(${this.state.tag.background}) 100% 100%`,
    ...
    >
     ...
     <div className='Header-Wrapper'>
      ...
      <div className='Header-Link Link'>
       <a href='/#'>Больше об {this.props.tag} ></a>
      </div>
     </div>
    </div>
   </>
   ...
   <>
    <div
     ...
     style={{
      background:
       'url(' +
       (!!this.props.backgroundImg ? this.props.backgroundImg : Background) +
       ') 100% 100%',
      backgroundSize: 'cover',
      backgroundRepeat: 'no-repeat',
     }}
